...
const YourPodcasts = () => {
  ...
  useEffect(() => {
    const fetchPodcasts = async () => {
      try {
        const res = await axios.get("https://podcast-backend-jgm2.onrender.com/api/v1/get-user-podcasts", {
          withCredentials: true,
        });
        setPodcasts(res.data.data);
      } catch (error) {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this podcast?")) {
      try {
        await axios.delete(`https://podcast-backend-jgm2.onrender.com/api/v1/delete-podcast/${id}`, { withCredentials: true });
        setPodcasts((prevPodcasts) => prevPodcasts.filter((podcast) => podcast._id !== id));
      } catch (error) {
        console.error("Error deleting podcast:", error);
        alert("Failed to delete the podcast. Please try again.");
