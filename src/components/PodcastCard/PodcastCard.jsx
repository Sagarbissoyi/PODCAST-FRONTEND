...
const handlePlay = (e) =>{
    ...
    dispatch(playerActions.changeImage(`https://podcast-backend-jgm2.onrender.com/${items.frontImage}`));
    dispatch(playerActions.changeSong(`https://podcast-backend-jgm2.onrender.com/${items.audioFile}`));
    }
};
    const handleDelete = () => {
        if (onDelete) {
    ...
    return (
        <div>
            <Link to={`/description/${items._id}`} 
                className='border p-4 rounded flex flex-col shadow-xl hover:shadow-2xl transition-all duration-300'>
                <div>
                    <img src={`https://podcast-backend-jgm2.onrender.com/${items.frontImage}`} 
                        className="rounded size[42vh] object-cover"
                    />
                </div>
                <div className="mt-2 text-xl font-bold">
